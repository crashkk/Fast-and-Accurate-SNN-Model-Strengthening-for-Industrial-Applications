...
from utils import *
from result_analize.metrics_packs import spike_cal
...
class SCNN(nn.Module):
    ...
    def forward(self, input,simulation_required=False, time_window = tw,batch_size=batch_size):
        ...
        for step in range(time_window): # simulation time steps
            ...
            if args.spikes_track==True:
                spc+=spike_cal(c1_spike,c2_spike,h1_spike,h2_spike)

        if args.spikes_track==True:
            self.spiking_frequecy+=spc
            print(self.spiking_frequecy)
